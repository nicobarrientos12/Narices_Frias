// src/pages/MascotaEnfermedadList.jsx
import React, { useEffect, useMemo, useState } from "react";
import { useNavigate, Link } from "react-router-dom";
import {
  getAllMascotaEnfermedades,
  eliminarMascotaEnfermedad,
...
export default function MascotaEnfermedadList() {
  ...
  const cargarRelaciones = async () => {
    try {
      setLoading(true);
      const data = await getAllMascotaEnfermedades();
  ...
  const handleDelete = async (id, mascota, enfermedad) => {
    if (!window.confirm(`Eliminar la relacion "${mascota} - ${enfermedad}"?`)) return;
    try {
      await eliminarMascotaEnfermedad(id);
      cargarRelaciones();
    } catch (err) {
  ...
  const groupedRelaciones = useMemo(() => {
    ...
    for (const r of relaciones) {
      ...
      map.get(key).items.push({
        ...
        fecha_diagnostico: r.fecha_diagnostico || null,
  ...
  const buildDescripcion = (group) => {
    return group.items.map((it) => {
      const parts = [];
      if (it.fecha_diagnostico) parts.push(`fecha: ${formatDate(it.fecha_diagnostico)}`);
      if (it.observaciones) parts.push(`obs: ${it.observaciones}`);
      return parts.length ? `${it.enfermedad} (${parts.join(', ')})` : `${it.enfermedad}`;
    }).join(' | ');
  };
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-extrabold" style={{ color: COLORS.ink }}>Enfermedades de mascotas</h2>
            <p className="text-sm" style={{ color: COLORS.inkSoft }}>Relaciones mascota - enfermedad</p>
          </div>
        </div>

        <Link
          to="/mascota-enfermedad/nueva"
      ...
      <div className="mb-4 flex flex-col md:flex-row md:items-center gap-3" style={{ color: COLORS.ink }}>
        <div className="relative w-full md:max-w-md">
          <input
            type="text"
            placeholder="Buscar por mascota, enfermedad u observaciones..."
            value={search}
        ...
        <table className="w-full hidden md:table">
          <thead style={{ background: "#FFFDF0" }}>
            <tr className="text-left text-sm font-semibold" style={{ color: COLORS.inkSoft }}>
              ...
              <th className="p-3 border-b" style={{ borderColor: COLORS.line }}>Enfermedades</th>
                        ...
                        <div className="grid gap-2">
                          {g.items.map((it) => (
                            <div key={it.id} className="flex items-center gap-3 flex-wrap">
                              <div className="font-semibold">{it.enfermedad}</div>
                              <div className="text-xs" style={{ color: COLORS.inkSoft }}>
                                {it.fecha_diagnostico ? `Fecha: ${formatDate(it.fecha_diagnostico)}` : 'Fecha: -'}
                              </div>
                              {it.observaciones && (
                                <div className="text-xs" style={{ color: COLORS.inkSoft }}>
                              ...
                              <div className="ml-auto inline-flex gap-1.5">
                                <button
                                  onClick={() => navigate(`/mascota-enfermedad/editar/${it.id}`)}
                                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg font-semibold"
                                  style={{ backgroundColor: "#FFF7D1", color: COLORS.ink, border: `1px solid ${COLORS.primary}` }}
                                  title="Editar"
                                >
                                ...
                                <button
                                  onClick={() => handleDelete(it.id, g.mascota, it.enfermedad)}
                                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg font-semibold"
                  ...
                  <div className="mt-3 grid gap-2">
                    {g.items.map((it) => (
                      <div key={it.id} className="rounded-xl border p-3" style={{ borderColor: COLORS.line }}>
                        <div className="font-semibold" style={{ color: COLORS.ink }}>{it.enfermedad}</div>
                        <div className="mt-2 text-xs" style={{ color: COLORS.inkSoft }}>
                          {it.fecha_diagnostico ? `Fecha: ${formatDate(it.fecha_diagnostico)}` : 'Fecha: -'}
                        </div>
                        {it.observaciones && (
