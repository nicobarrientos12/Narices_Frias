// src/pages/TratamientoList.jsx
import React, { useEffect, useMemo, useState } from "react";
import { useNavigate } from "react-router-dom";
import { getAllTratamientos, eliminarTratamiento } from "../../services/tratamientoService";
...
export default function TratamientoList() {
  ...
  // Normaliza payload del servicio (array directo o {data}/otros)
  ...
  const cargarTratamientos = async () => {
    try {
      setLoading(true);
      // 👇 El servicio puede devolver array directo; no lo desestructures como { data }
      const payload = await getAllTratamientos();
  ...
  const handleDelete = async (id, mascota) => {
    if (!window.confirm(`¿Eliminar el tratamiento de "${mascota}"?`)) return;
    try {
  ...
  // Búsqueda (mascota o diagnóstico o veterinario)
  const filteredTratamientos = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return tratamientos;
    return tratamientos.filter(
      (t) =>
        (t.mascota || "").toLowerCase().includes(q) ||
        (t.diagnostico || "").toLowerCase().includes(q) ||
        (t.veterinario || "").toLowerCase().includes(q)
    );
  }, [tratamientos, search]);

  // Paginación
  const totalPages = Math.max(1, Math.ceil(filteredTratamientos.length / limit));
    ...
    <div className="p-6"
      ...
      <div className="mb-5 rounded-2xl px-4 py-3 flex items-center justify-between"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-extrabold" style={{ color: COLORS.ink }}>Tratamientos médicos</h2>
            <p className="text-sm" style={{ color: COLORS.inkSoft }}>Historial de atención</p>
          </div>
        </div>
      ...
      <div className="mb-4 flex flex-col md:flex-row md:items-center gap-3" style={{ color: COLORS.ink }}>
        <div className="relative w-full md:max-w-md">
          ...
          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs px-2 py-1 rounded-lg"
            ...
            {filteredTratamientos.length} resultado(s)
          </span>
        </div>
      ...
      <div className="overflow-x-auto rounded-2xl"
        ...
        <table className="w-full hidden md:table">
          <thead style={{ background: "#FFFDF0" }}>
            <tr className="text-left text-sm font-semibold" style={{ color: COLORS.inkSoft }}>
              ...
              <th className="p-3 border-b" style={{ borderColor: COLORS.line }}>Diagnóstico</th>
              <th className="p-3 border-b" style={{ borderColor: COLORS.line }}>Inicio</th>
              <th className="p-3 border-b" style={{ borderColor: COLORS.line }}>Fin</th>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="7" className="p-6 text-center text-sm" style={{ color: COLORS.inkSoft }}>Cargando tratamientos…</td></tr>
            ) : paginatedTratamientos.length ? (
              paginatedTratamientos.map((t) => (
                <tr key={t.id} className="align-middle text-sm transition hover:bg-[#FFFCF2]" style={{ color: COLORS.ink }}>
                  <td className="p-3 border-t font-semibold" style={{ borderColor: COLORS.line }}>{t?.mascota || "—"}</td>
                  <td className="p-3 border-t" style={{ borderColor: COLORS.line }}>{t?.veterinario || "—"}</td>
                  ...
                  <td className="p-3 border-t" style={{ borderColor: COLORS.line, color: COLORS.inkSoft }}>
                    <div className="flex items-center gap-2">
                      <span>{formatDate(t?.fecha_fin)}</span>
                      <DuracionChip inicio={t?.fecha_inicio} fin={t?.fecha_fin} />
                    </div>
                  </td>
                  <td className="p-3 border-t" style={{ borderColor: COLORS.line }}>{formatMoney(t?.precio)}</td>
                  <td className="p-3 border-t text-center" style={{ borderColor: COLORS.line }}>
                    <div className="inline-flex gap-1.5">
                      <button
                        onClick={() => navigate(`/tratamientos/editar/${t.id}`)}
                      ...
                      <button
                        onClick={() => handleDelete(t.id, t.mascota)}
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg font-semibold"
              ...
              <tr>
                <td colSpan="7" className="p-10 text-center">
                  ...
                  <div className="font-bold" style={{ color: COLORS.ink }}>No hay tratamientos registrados</div>
        ...
        <div className="md:hidden divide-y" style={{ borderTop: `1px solid ${COLORS.line}` }}>
          {loading ? (
            <div className="p-6 text-center text-sm" style={{ color: COLORS.inkSoft }}>Cargando tratamientos…</div>
          ) : paginatedTratamientos.length ? (
            paginatedTratamientos.map((t) => (
              <div key={t.id} className="p-4">
                ...
                <div className="mt-3 grid grid-cols-1 gap-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-[13px]" style={{ color: COLORS.inkSoft }}>Inicio</span>
                    <span style={{ color: COLORS.inkSoft }}>{formatDate(t?.fecha_inicio)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[13px]" style={{ color: COLORS.inkSoft }}>Fin</span>
                    <span style={{ color: COLORS.inkSoft }}>
                      {formatDate(t?.fecha_fin)} <DuracionChip inicio={t?.fecha_inicio} fin={t?.fecha_fin} />
                    </span>
                  </div>
                  {t?.diagnostico && (
