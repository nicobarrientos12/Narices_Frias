// src/pages/AplicacionesList.jsx
import React, { useEffect, useMemo, useState } from "react";
import { useNavigate } from "react-router-dom";
import { getAllAplicaciones, eliminarAplicacion } from "../../services/mascotaVacunaService";
...
function ProximaBadge({ proxima_aplicacion }) {
  const today = new Date();
  const diff = daysBetween(today, proxima_aplicacion);
  let bg = "#F3F4F6", border = "#E5E7EB", text = "#374151", label = "—";
  if (diff === null) {
    label = "Sin fecha";
  } else if (diff < 0) {
    bg = "#FEF2F2"; border = "#FCA5A5"; text = "#991B1B"; label = "Vencida";
  } else if (diff <= 7) {
    bg = "#FFFBEB"; border = "#FDE68A"; text = "#92400E"; label = "Próxima";
  } else {
    bg = "#ECFDF5"; border = "#A7F3D0"; text = "#065F46"; label = "Al día";
  }
  return (
...
export default function AplicacionesList() {
  ...
  const cargarAplicaciones = async () => {
    try {
      setLoading(true);
      const data = await getAllAplicaciones();
  ...
  const handleDelete = async (id, mascota, vacuna) => {
    if (!window.confirm(`¿Eliminar la aplicación de ${vacuna} para "${mascota}"?`)) return;
    try {
  ...
  // Agrupar por mascota para mostrar historial en un solo registro
  const groupedAplicaciones = useMemo(() => {
    const map = new Map();
    for (const a of aplicaciones) {
      const key = a.mascota_id ?? a.mascota ?? a.id;
      if (!map.has(key)) {
        map.set(key, {
          key,
          mascota: a.mascota || '-',
      ...
      map.get(key).items.push({
        ...
        proxima_aplicacion: a.proxima_aplicacion || null,
      });
    }
    return Array.from(map.values()).sort((x, y) =>
      (x.mascota || '').localeCompare(y.mascota || '', 'es')
    );
  }, [aplicaciones]);
  ...
  const buildDescripcion = (group) => {
    return group.items.map((it) => {
      const parts = [];
      if (it.fecha_aplicacion) parts.push(`aplicacion: ${formatDate(it.fecha_aplicacion)}`);
      if (it.proxima_aplicacion) parts.push(`proxima: ${formatDate(it.proxima_aplicacion)}`);
      return parts.length ? `${it.vacuna} (${parts.join(', ')})` : `${it.vacuna}`;
    }).join(' | ');
  };

  // Busqueda (mascota o vacuna)
  const filteredAplicaciones = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return groupedAplicaciones;
    return groupedAplicaciones.filter((g) =>
      (g.mascota || '').toLowerCase().includes(q) ||
      buildDescripcion(g).toLowerCase().includes(q)
    );
  }, [groupedAplicaciones, search]);

  // Paginación
  const totalPages = Math.max(1, Math.ceil(filteredAplicaciones.length / limit));
  const paginatedAplicaciones = filteredAplicaciones.slice(
    (currentPage - 1) * limit,
    currentPage * limit
  );
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-extrabold" style={{ color: COLORS.ink }}>Aplicaciones de Vacunas</h2>
            <p className="text-sm" style={{ color: COLORS.inkSoft }}>Control de dosis por mascota</p>
          </div>
        </div>

        <button
          onClick={() => navigate("/vacunas/nueva")}
      ...
      <div className="mb-4 flex flex-col md:flex-row md:items-center gap-3" style={{ color: COLORS.ink }}>
        <div className="relative w-full md:max-w-md">
          ...
          >
            {filteredAplicaciones.length} resultado(s)
          </span>
        </div>
      ...
      >
        <table className="w-full hidden md:table">
          <thead style={{ background: "#FFFDF0" }}>
            <tr className="text-left text-sm font-semibold" style={{ color: COLORS.inkSoft }}>
              ...
              <th className="p-3 border-b" style={{ borderColor: COLORS.line }}>Vacunas</th>
          ...
          <tbody>
            {loading ? (
            ...
            ) : paginatedAplicaciones.length ? (
              paginatedAplicaciones.map((g) => (
                    ...
                    <td className="p-3 border-t font-semibold" style={{ borderColor: COLORS.line }}>
                      ...
                      {(g?.especie || g?.raza) ? (
                        <div className="text-xs" style={{ color: COLORS.inkSoft }}>
                          {[g.especie, g.raza].filter(Boolean).join(" - ")}
                        </div>
                      ) : null}
                    </td>
                        ...
                        <div className="grid gap-2">
                          {g.items.map((it) => (
                            <div key={it.id} className="flex items-center gap-3 flex-wrap">
                              <div className="font-semibold">{it.vacuna}</div>
                              {it.lote && (
                                <div className="text-xs" style={{ color: COLORS.inkSoft }}>Lote: {it.lote}</div>
                              )}
                              <div className="text-xs" style={{ color: COLORS.inkSoft }}>
                                {it.fecha_aplicacion ? `Aplicacion: ${formatDate(it.fecha_aplicacion)}` : 'Aplicacion: -'}
                              </div>
                              <div className="text-xs" style={{ color: COLORS.inkSoft }}>
                                {it.proxima_aplicacion ? `Proxima: ${formatDate(it.proxima_aplicacion)}` : 'Proxima: -'}
                              </div>
                              <div className="ml-auto inline-flex items-center gap-2">
                                <ProximaBadge proxima_aplicacion={it.proxima_aplicacion} />
                                <button
                                  onClick={() => navigate(`/vacunas/editar/${it.id}`)}
                                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg font-semibold"
                                  style={{ backgroundColor: "#FFF7D1", color: COLORS.ink, border: `1px solid ${COLORS.primary}` }}
                                  title="Editar"
                                >
                                ...
                                <button
                                  onClick={() => handleDelete(it.id, g.mascota, it.vacuna)}
                                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg font-semibold"
              ...
              <tr>
                <td colSpan="4" className="p-10 text-center">
                  ...
                  <div className="font-bold" style={{ color: COLORS.ink }}>No se encontraron aplicaciones</div>
        ...
        <div className="md:hidden divide-y" style={{ borderTop: `1px solid ${COLORS.line}` }}>
          {loading ? (
            <div className="p-6 text-center text-sm" style={{ color: COLORS.inkSoft }}>Cargando aplicaciones...</div>
          ) : paginatedAplicaciones.length ? (
            paginatedAplicaciones.map((g) => (
                  ...
                  <div className="mt-3 grid gap-2">
                    {g.items.map((it) => (
                      <div key={it.id} className="rounded-xl border p-3" style={{ borderColor: COLORS.line }}>
                        <div className="font-semibold" style={{ color: COLORS.ink }}>{it.vacuna}</div>
                        {it.lote && (
                          <div className="mt-1 text-xs" style={{ color: COLORS.inkSoft }}>Lote: {it.lote}</div>
                        )}
                        <div className="mt-2 text-xs" style={{ color: COLORS.inkSoft }}>
                          {it.fecha_aplicacion ? `Aplicacion: ${formatDate(it.fecha_aplicacion)}` : 'Aplicacion: -'}
                        </div>
