// src/pages/Alergia/AlergiaList.jsx
import React, { useEffect, useMemo, useState } from "react";
import { useNavigate } from "react-router-dom";
import { getAlergias, deleteAlergia } from "../../services/AlergiaService";
...
export default function AlergiaList() {
  ...
  const cargarAlergias = async () => {
    try {
      setLoading(true);
      // ⬇️ JSON directo desde el service
  ...
  useEffect(() => { cargarAlergias(); }, []);

  const handleDelete = async (id, nombre) => {
    if (!window.confirm(`¿Eliminar la alergia "${nombre}"?`)) return;
    try {
  ...
  const filteredAlergias = useMemo(() => {
    ...
    if (!q) return alergias;
    return alergias.filter((a) => (a.nombre || "").toLowerCase().includes(q));
  }, [alergias, search]);

  const totalPages = Math.max(1, Math.ceil(filteredAlergias.length / limit));
  const paginatedAlergias = filteredAlergias.slice((currentPage - 1) * limit, currentPage * limit);
    ...
    <div className="p-6" style={{ fontFamily: '"Plus Jakarta Sans","Inter","Noto Sans",system-ui,sans-serif', backgroundColor: "#FAFAFA" }}>
      ...
      <div className="mb-5 rounded-2xl px-4 py-3 flex items-center justify-between" style={{ backgroundColor: COLORS.bg, border: `1px solid ${COLORS.line}` }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-extrabold" style={{ color: COLORS.ink }}>Alergias</h2>
            <p className="text-sm" style={{ color: COLORS.inkSoft }}>Catálogo y mantenimiento de alergias</p>
          </div>
        </div>

        <button
          onClick={() => navigate("/alergias/nuevo")}
      ...
      <div className="mb-4 flex flex-col md:flex-row md:items-center gap-3" style={{ color: COLORS.ink }}>
        <div className="relative w-full md:max-w-md">
          ...
          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs px-2 py-1 rounded-lg" style={{ background: COLORS.chip, border: `1px solid ${COLORS.line}`, color: COLORS.inkSoft }}>
            {filteredAlergias.length} resultado(s)
          </span>
        </div>
      ...
      >
        <table className="w-full hidden md:table">
          ...
          <tbody>
            {loading ? (
            ...
            ) : paginatedAlergias.length ? (
              paginatedAlergias.map((a) => (
                <tr key={a.id} className="align-middle text-sm transition hover:bg-[#FFFCF2]" style={{ color: COLORS.ink }}>
                  ...
                  <td className="p-3 border-t text-center" style={{ borderColor: COLORS.line }}>
                    <div className="inline-flex gap-1.5">
                      <button
                        onClick={() => navigate(`/alergias/editar/${a.id}`)}
              ...
              <tr>
                <td colSpan="3" className="p-10 text-center">
                  ...
                  <div className="font-bold" style={{ color: COLORS.ink }}>No se encontraron alergias</div>
        ...
        <div className="md:hidden divide-y" style={{ borderTop: `1px solid ${COLORS.line}` }}>
          {loading ? (
            <div className="p-6 text-center text-sm" style={{ color: COLORS.inkSoft }}>
              Cargando alergias…
            </div>
          ) : paginatedAlergias.length ? (
            paginatedAlergias.map((a) => (
